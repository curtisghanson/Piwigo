...
use Symfony\Component\Security\Core\User\AdvancedUserInfterface;

class User implements AdvancedUserInfterface
{
    const ENABLED      = 0; // user access is enabled
    const CRED_EXPIRED = 1; // for password expire rules
    const EXPIRED      = 2; // for temp accounts to expire
    const LOCKED       = 3; // for acct temp loc, admins and above
    const DISABLED     = 4; // for acct perm lock, super_admins only
    ...
    private $statuses = array(
        ...
        self::CRED_EXPIRED => 'cred_expired',
    ...
    {
        $this->roles       = \SplObjectStorage();
        $this->groups      = \SplObjectStorage();
        $this->metadata    = \SplObjectStorage();
        $this->preferences = \SplObjectStorage();
    }
