...
{
  global $conf, $user, $persistentCache;

  $cache_key = $persistentCache->makeKey('recent_posts'.$user['id'].$user['cache_update_time'].$max_dates.$max_elements.$max_cats);
  if ($persistentCache->get($cache_key, $cached))
  {
    return $cached;
  }
...
;';
  ...
  $persistentCache->set($cache_key, $dates);
  return $dates;
}
