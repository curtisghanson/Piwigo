...
use Piwigo\Http\Request;
use Piwigo\Http\Response;
...
function rate_picture($image_id, $rate)
{
  ...
  $anonymous_id = implode ('.', $ip_components);
    ...
    {
        $request = new Request();
        $save_anonymous_id = $request->getCookie('anonymous_rater', $anonymous_id);

        if ($anonymous_id != $save_anonymous_id)
        {
            // client has changed his IP adress or he's trying to fool us
            $query = 'SELECT element_id FROM ' . RATE_TABLE . ' WHERE user_id = ' . $user['id'] . ' AND anonymous_id = \'' . $anonymous_id . '\';';
...
;';
       pwg_query($query);
        ...
        $response->setCookie('anonymous_rater', $anonymous_id);
    } // end anonymous user
