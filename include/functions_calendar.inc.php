...
{
  global $page, $conf, $user, $template, $persistentCache, $filter;
...
//-------------------------------------- initialize the calendar parameters ---
  ...
  if ($must_show_list)
  {
    ...
    {
      $cache_key = $persistentCache->makeKey($user['id'].$user['cache_update_time']
        .$calendar->date_field.$order_by);
    }

    if ( !isset($cache_key) || !$persistentCache->get($cache_key, $page['items']))
    {
  ...
  '.$order_by;
      ...
      if (isset($cache_key))
        $persistentCache->set($cache_key, $page['items']);
    }
  }
